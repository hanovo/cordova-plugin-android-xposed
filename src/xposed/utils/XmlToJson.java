...
public class XmlToJson {
  ...
  // default values when a XmlTag is empty
  ...
  JSONObject convertToJSONObject() {
    try {
      XmlTag parentTag = new XmlTag("", "xml");
  ...
  private void readTags(XmlTag parent, XmlPullParser xpp) {
    try {
      int eventType;
      do {
        eventType = xpp.next();
        if (eventType == XmlPullParser.START_TAG) {
          ...
          XmlTag child = new XmlTag(path, tagName);
          if (!skipTag) {
          ...
          for (int i = 0; i < attrCount; ++i) {
            ...
            XmlTag attribute = new XmlTag(attrPath, attrName);
  ...
  private JSONObject convertTagToJson(XmlTag tag, boolean isListElement) {
    ...
    try {
      ...
      HashMap<String, ArrayList<XmlTag>> groups = tag.getGroupedElements(); // groups by tag names so that we can detect lists or single elements
      for (ArrayList<XmlTag> group : groups.values()) {

        if (group.size() == 1) {    // element, or list of 1
          XmlTag child = group.get(0);
  ...
  private boolean isForcedList(XmlTag tag) {
    String path = tag.getPath();
